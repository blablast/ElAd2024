...
namespace ElAd2024.ExtensionMethods;
...
public static class ExtensionMethodsSerialDevice
{
    public static void CopyTo(this SerialDevice serialDevice, SerialPortInfo serialPortInfo)
    {
        ...
        foreach (var spDeviceProp in serialDeviceProperties)
        {
            ...
            var spInfoProp = serialPortInfoProperties.FirstOrDefault(p =>
                p.Name == spDeviceProp.Name && p.PropertyType == spDeviceProp.PropertyType && p.CanWrite &&
                p.GetCustomAttribute<OmitPropertyAttribute>() is null);

            if (spInfoProp is null)
            {
                continue;
            }
